import paddle
from paddle.nn import LayerList, Linear, LayerNorm
...
class TransformerDecoderLayer(paddle.nn.Layer):
    def __init__(self, d_model, nhead, vocab_size, dim_feedforward=2048, dropout=0.1, activation='relu'):
        ...
        self.cross_attn = paddle.nn.MultiHeadAttention(d_model, nhead, dropout=dropout)
...
class TransformerDecoder(paddle.nn.Layer):
    def __init__(
        ...
        """
        super().__init__()
        self.embed = paddle.nn.Embedding(vocab_size, d_model)
        self.linear = Linear(d_model, vocab_size)
        self.layers = LayerList([TransformerDecoderLayer(d_model, nhead, vocab_size, dim_feedforward, dropout, activation) for _ in range(num_layers)])
    ...
    def forward(self, tgt, memory, **kwargs):
        ...
        - 返回最后一步预测概率分布
        """
        tgt = self.embed(tgt)    #词嵌入层，只是为了演示推理过程的beam search,所以用词嵌入作为decoder输入
...
class GreedySearch:
    ...
    def decode(self, start_id, end_id):
        ...
        """
        action_seq = paddle.to_tensor([[start_id]])  # 初始化解码序列
        time_step = 0
        while time_step < self.max_len:  # 控制解码步数，防止过长
            ...
            if paddle.argmax(logits) == end_id:  # 如果预测到结束标识符，则终止解码
                break
            action_seq = paddle.concat((action_seq, paddle.argmax(logits).unsqueeze([0, 1])), axis=-1)  # 根据概率分布选择最大概率的字符并添加到解码序列中
            time_step += 1
...
class BeamSearch:
    ...
    def decode(self, start_id, end_id):
        ...
        """
        # 初始化活跃序列
        active_sequences = [(paddle.to_tensor([[start_id]]), paddle.log(paddle.to_tensor([1.0])), False)]
        time_step = 0        
        while len(active_sequences) > 0 and time_step < self.max_len:
            ...
            for previous_sequence, previous_score , finished in active_sequences:
                ...
                # 在概率分布上进行Beam搜索
                topk_probs, topk_ids = paddle.topk(next_probs, k=self.beam_width, axis=-1)
                for id in topk_ids.squeeze().numpy():
                    if id == end_id:  # 如果生成了结束标记，则结束该序列
                        new_sequences.append((paddle.concat([previous_sequence, paddle.unsqueeze(paddle.to_tensor([id]), axis=-1)], axis=-1), previous_score + paddle.to_tensor([next_probs.squeeze().numpy()[id]]), True))
                    else:  # 继续生成序列
                        new_sequences.append((paddle.concat([previous_sequence, paddle.unsqueeze(paddle.to_tensor([id]), axis=-1)], axis=-1), previous_score + paddle.to_tensor([next_probs.squeeze().numpy()[id]]), False))
        ...
        sorted_sequences = sorted(active_sequences, key=lambda x: x[1], reverse=True)
        return sorted_sequences[0][0].numpy().flatten().tolist()
    
# 以下代码演示了如何使用两种不同的搜索策略：贪心搜索（Greedy Search）和束搜索（Beam Search），
# 来解码来自一个记忆张量（memory_tensor）的信息。

# 示例用法:
memory_tensor = paddle.randn((1, 10, 512))  # 从编码器获取的记忆张量，维度为(批大小, 序列长度, 特征维度)
...
# 使用束搜索策略
beam_search = BeamSearch(memory_tensor)
print(beam_search.decode(0, 1))  # 同样对记忆张量进行解码，但使用不同的搜索策略